 ...
 *
 * <p>This resolver assigns deterministic ids for messages and fields, preserving existing ids from
 * a lock file when available.
 */
public final class SchemaResolver {
     ...
     *
     * @param rawSchema the raw schema definition
     * @param existingLockFile the existing lock file, or null if none
     * @param schemaPath the source path for metadata in the lock file
     * @return the resolution result containing resolved schema and lock data
     */
    ...
    private ResolutionResult runResolution(Path schemaPath) {
        // Validate that any existing lock file refers to the same schema namespace
        if (lockFile != null && lockFile.schemaInfo != null) {
    ...
    private LockFile buildUpdatedLockFile(Path schemaPath) {
        ...
        Path sourcePath = schemaPath.getFileName();
        String sourceFile = sourcePath != null ? sourcePath.toString() : schemaPath.toString();
        result.schemaInfo =
                Map.of(
                        "namespace",
                        rawSchema.namespace(),
