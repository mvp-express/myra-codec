...
/** Tests schema resolution and lock file behavior. */
class SchemaResolverTest {
    ...
    @Test
    void resolve_ShouldPreserveReservedMessageIdsFromLockFile() {
        SchemaDefinition schema =
                new SchemaDefinition(
                        "com.test",
                        "1.0.0",
        ...
        deletedMsg.fields = Map.of("field1", 1);
        existingLock.messages = Map.of("DeletedMessage", deletedMsg);
        existingLock.reservedIds = Map.of("messages", List.of(1, "bad", 3));
        ...
        List<Integer> reserved =
                (List<Integer>) result.updatedLockFile().reservedIds.get("messages");
        assertNotNull(reserved);
        assertTrue(reserved.contains(5));
        assertTrue(reserved.contains(1));
        assertTrue(reserved.contains(3));
    }
